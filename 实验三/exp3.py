...
def init_k(dataMatrix,k,num):   # 初始化k值(数据源,k值，)
    k_point={}
    record= []
    while(len(record)!=k):
...
def upgrade(point_tag,k_point,k,num,new_dataMatrix,p_num,coord_num):    # 修改中心点
    for i in range(0,k):
...
def k_means(dataMatrix,coord_num,num,k):
    '''
    聚类->修改中心点->在聚类->再修改中心点->两次的总的距离一样就停止
    '''
...
def experiment(dataMatrix,k):
    mean_point = {i:0 for i in range(10)}
    ...
    k_means(new_dataMatrix,coord_num,num,k)
    ...
    point_tag,k_point = k_means(dataMatrix,coord_num,num,k)
