...
def mean_list(list)->float:   # 返回列表平均值，计算时跳过空缺值
    sum = float(0)
    n = float(len(list))
    for num in list:
        if math.isnan(num):
...
def SD_list(list,mean):         #返回列表的标准差，计算时跳过空缺值  
    #均值为nan直接返回nan
    if math.isnan(mean):
        return float('nan')
...
if __name__ == '__main__':
    ...
    plt.savefig('D:\大三上\数据挖掘\exp2.4.png',bbox_inchex='tight')
    plt.show()
